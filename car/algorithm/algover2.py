...
def escape(esCount):
    ...
    if direction ==  10000:
        ...
        while ( esCount == 0 or distance() > 2000 or distance() < 5 ):
            for i in range(0,3):
                down()
            pause()
            time.sleep(0.1)
        ...
        print("here!!!")
        if ( esCount == 0 or distance() > 2000 or distance() < 5 ):
            print("remain stuck!")
            esCount += 1
            escape(esCount)
...
def isBlock(cnt):
    print('here111')
     if ( infraLeftSide == GPIO.LOW and infraRightSide == GPIO.LOW) :
        #左右两侧被挡，就先后退
        print("block!",cnt)
...
def checkCrush():
    if ( GPIO.input(leftCrush) == GPIO.HIGH ):
        for i in range(0,3):
            CW()
    if ( GPIO.input(rightCrush) == GPIO.HIGH ):
        for i in range(0,3):
            CCW()
...
def checkBlock():
    ...
    dist = distance()
    print(dist)
    if ( distance() < 5 or distance() > 2000 ):
        for i in range(0,3):
                CW()
    print('here111')    
    isBlock(cnt)
